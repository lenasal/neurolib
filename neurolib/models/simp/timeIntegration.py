...
def timeIntegration(params, control):
    ...
    ext_current_e = np.zeros((N, len(t)+1))
...
def timeIntegration_njit_elementwise(
    ...
    for i in range(1,len(t)+1):
        for no in range(N):
            ...
            xid1, yid1, dxid, dyid = fast_interp2_opt(sigmarange, ds, 1.5, Irange, dI, mufe[no,i])
            ...
            #rates_exc[no,i] = 25.*mufe[no,i]
            rates_exc[no,i] = mufe[no,i-1]
            tau_exc[no,i] = 1.
            
            mufe_rhs = ( ext_current_e[no,i] + control_ext[no,0,i] ) / tau_exc[no,i]#- mufe[no,i-1] ) / tau_exc[no,i]
