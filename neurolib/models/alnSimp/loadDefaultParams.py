...
def loadDefaultParams(Cmat=None, Dmat=None, lookupTableFileName=None):
    ...
    params.model = "alnSimp"
    params.name = "alnSimp"
    params.description = "Simplified version of aln for testing purposes"
    ...
    params.ext_exc_current = 1.0  # external excitatory input current [mV/ms], C*[]V/s=[]nA
    params.ext_inh_current = 1.0
    ...
    params.rates_inh_init = np.array( [[0.]] )

    params.mufe_init = np.array( [[0.]] )  # (linear) filtered mean input
    params.rates_exc_init = np.zeros((1,1))  #
    
    params.mufi_init = np.array( [[0.]] )  # (linear) filtered mean input
    params.rates_inh_init = np.zeros((1,1))  #
