...
def phi(model, state_, target_state_, control_, phi_prev_, start_ind_ = 0):
    ...
    for ind_time in range(phi_.shape[2]-1, start_ind_-1, -1):
        ...
        phi_[0,0,ind_time] = - full_cost_grad[0] - np.dot( np.array( [phi_[0,2,ind_time], phi_[0,4,ind_time], phi_[0,5,ind_time],
                            phi_[0,9,ind_time], phi_[0,15,ind_time] ] ), np.array( [jac[2,0], jac[4,0], jac[5,0], jac[9,0], jac[15,0]] ) )
        ...
        if (ind_time != phi_.shape[2]-1):
            ...
            der = phi_[0,2,ind_time] * jac[2,9] + phi_[0,9,ind_time] * jac[9,9] + phi_[0,15,ind_time] * jac[15,9]
...
def jacobian(model, state_, control_, t_):
    ...
    jacobian_[2,18] = ( Jee_max * state_[0,5,t_-1] + control_[0,0,t_] + ext_exc_current - state_[0,2,t_-1] ) / state_[0,18,t_-1]**2
    ...
    sigma_sqrt = ( state_[0,9,t_] * ( 2. * Jee_max**2 * tau_se * taum ) * ( (1 + z1ee) * taum + tau_se )**(-1) + sigmae_ext**2 )**(-1./2.)
    
    jacobian_[15,0] = 0.5 * (1e-3) * factor_ee1 * taum * ( (1 + z1ee) * taum + tau_se )**(-2) * state_[0,9,t_] * ( 2. * Jee_max**2 * tau_se * taum ) * sigma_sqrt
    jacobian_[15,9] = - 0.5 * ( (1 + z1ee) * taum + tau_se )**(-1) * ( 2. * Jee_max**2 * tau_se * taum ) * sigma_sqrt
    jacobian_[15,15] = 1.
